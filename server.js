...
app.post("/api/eventdesc", (req, res) => {
  ...
  createEventProposals.createDescription(
    db,
    req.body.eventInput[0],
...
app.get("/api/testrender", (req, res) => {
  ...
  renderAvailability
    ...
    .then(response => {
      ...
      rsvpNameInfo.rsvpNameInfo(
        db,
        req.query.rsvpData[0],
        req.query.rsvpData[1]
      );
    })
    .catch(error => {
      console.log(error);
...
app.post("/api/insertvote", (req, res) => {
  ...
  voteInsert.voteInsert(
    db,
    req.body.proposalData[0],
    req.body.proposalData[1],
    req.body.proposalData[2],
    req.body.proposalData[3]
  );
  res.send("boo");
});
